...
export default function Data({data}) {
  ...
  return (
        ...
        {sims.map((sim,index) => 
          ...
          <p>Gender: {sim.gender.genderName}</p>
          {sim.ageGroup.id >= 2 && <p>Aspiration: {sim.aspiration.aspirationName}</p>}
          ...
          <p>Skills to max: {sim.skills.map((skill) => <li key={skill.skillName}>{skill.skillName}</li>)}</p>
          {sim.ageGroup.id >= 3 && <p>Sexual preference: {sim.sexualPreference.sexualityName}</p>}

...
export default function Data({data}) {
  ...
  return (
        ...
        <div id='weather-text'>
        <div className='temp'>Lifespan: {lifeSpan.spanName}</div>
        <p>Difficulty: {difficulty}</p>
        <p>Age: {sims[0].ageGroup.name}</p>
        <p>Gender: {sims[0].gender.genderName}</p>
        {sims[0].ageGroup.id >= 2 && <p>Aspiration: {sims[0].aspiration.aspirationName}</p>}
        ...
        <p>Skills to max: {sims[0].skills.map((skill) => <li key={skill.skillName}>{skill.skillName}</li>)}</p>
        {sims[0].ageGroup.id >= 3 && <p>Sexual preference: {sims[0].sexualPreference.sexualityName}</p>}
